...
load_dotenv()
webhook_url = os.environ.get('DISCORD_KIM_REGISTRY_WEBHOOK')
...
def webhooks(title, author):
    ...
    embed.add_embed_field(name="Nhà xuất bản",
                          value="NXB Kim Đồng", inline=False)
    embed.set_timestamp()
...
latest_registry = ''
try:
    with open("latest_kim.txt", "r") as file:
...
registries = crawl(8, 5)  # 8 = KIM DONG
...
with open("latest_kim.txt", "wt") as file:
